<template>
  <div class="sb__container">
        ...
        <div class="sb__option">
          <header class="option__header">
            ...
            <NotificationPopup>
              <p class="sb__guide__text">
                * 작성하려는 주의 일지를
                <em>[개인별 주간] => [내 일정만 보기] => [엑셀]</em>로 받아서 등록해주세요.
              </p>
          ...
          <div class="option__content">
            <p class="sb__options__guide">
              <em>[개인별 주간] => [내 일정만 보기] => [엑셀]</em> 순으로 Excel파일을 받아, 불러와 주세요.
            </p>
    ...
    <div class="sb__result">
      <div class="result__chart result__chart--bar" v-if="Object.keys(barChartData).length">
        <Chart :data="barChartData" :options="barChartOptions" type="bar" />
      </div>
      <div class="result__chart result__chart--round" v-if="Object.keys(roundChartData).length">
        <Chart :data="roundChartData" :options="roundChartOptions" type="round" />
      </div>
      ...
      <div class="result__summary" v-if="Object.keys(summaryData).length">
        <div class="summary__box">
          <h3 class="summary__header">
            요약
            <ButtonElement type="button" class="summary__copy" size="xxs" line="black" @on-click="onCopyText"
              >복사</ButtonElement
            >
          </h3>
  ...
  export default {
    ...
    setup() {
      ...
      watch(workEndTime, () => {
        ...
        filteredExcelData.value = filterWeekAllExcel(excelData.value, workEndTime.value);
        barChartData.value = convertToBarChartData(filteredExcelData.value);
        roundChartData.value = convertToRoundChartData(filteredExcelData.value);
        summaryData.value = getSummaryData(filteredExcelData.value);
        summaryTotalData.value = getSummaryTotalData(filteredExcelData.value);
